...
public class InterfaceConsole {
    ...
    public void exibirAlerta(String msg) {
        System.out.println("[!] [RISCO/ALARME] " + msg);
    }
    ...
    public void exibirPainelProjeto(GestaoProjeto projetoAtual) {
        ...
        System.out.println("| ORÇAMENTO   : R$ " + String.format("%.2f", projetoAtual.getOrcamento()));
        System.out.println("| PRIORIDADE  : " + projetoAtual.getPrioridade());
    ...
    public void exibirPainelTime(Time timeAtual) {
        ...
        timeAtual.getMembros().forEach(membroA -> System.out
                .println("|   - " + membroA.getNome() + " [Acesso: " + membroA.getPerfil() + "]"));
        System.out.println("| CARTEIRA DE PROJETOS E INTERVENÇÕES:");
        timeAtual.getProjetos().forEach(projetoAtual -> System.out.println("|   - "
                + projetoAtual.getNome() + " [Situação Plena: " + projetoAtual.getStatus() + "]"));
        System.out.println("+" + LINHA_EXECUTIVA + "+");
    }
}
